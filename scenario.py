...
def get_intro_text():
    ...
    text = "At that time, I was working hard on that famous Topological Astronomy Problem and I began to inattentively read the message..."+\
            ...
            "Take all my money. Buy a ship, hire men, prepare a big exploration trip. Find Perigeo's shipwreck, pursue his travel," + \
            "and finish what he began. Bring back with you a keepsake of Perigeo. He deserves it.\n\n        You cannot let him in here."
...
def get_instructions():
    title = thorpy.make_text("Instructions", thorpy.style.TITLE_FONT_SIZE, gui.TCOLOR)
    ...
    #
    ftext = "Flags: You have " + str(parameters.MAX_FLAGS) + " flags at your disposal to help you locate. " + \
            "Press F to plant a flag on the ground. You can remove flags. Flags automatically add entries in logbook."
    ...
    watertext = "Water: Crew has to drink water in order to survive. Remaining water is schematized by a gauge on the right. Low temperature result in lower demand in food. "+\
                "Ship can carry a limited amount of water based on fresh water tank size, as well as the crew when walking. You can store food from/into villages. " +\
                "You can obtain water by purchasing a watermaker, or to anchor at villages."
    ewater = make_instruction(watertext)
    
    #
    huntext = "Hunting: Crew members are able to hunt if there are arrows in your stock. Successful hunting refill your food bar." + \
              "The chance to find food depends on the temperature and the skills of the best hunter in your crew."
    ehunt = make_instruction(huntext)
    #
    vtext = "Village: Villages contains and produce food and water throughout the year. You can exchange food with villages by pressing 'E'." +\
            "Refill your water by pressing 'W'. However, villages become more rare as you go far from civilization."
    evillage = make_instruction(vtext)
    #
    camptext = "Camp: You can camp/uncamp by pressing C. You have " + str(parameters.MAX_CAMPS) + " camps at your disposal." + \
                "You need a camp to hunt and to wait. As for villages, you can exchange food with camps."
    ecamp = make_instruction(camptext)
    #
    wtext = "Weather: Altitude influe temperature. Wind is schematized by a line over the compass. Wind moves the ship. " +\
            "Sometimes, violent storms can appear."
    eweather = make_instruction(wtext)
    #
    #
    waitext = "Waiting: You can wait for a chosen duration (in days) if you are in a camp. Crew still consume food during waiting." + \
              "Waiting will be stopped if food level reaches zero. Press P."
    ewait = make_instruction(waitext)
    #
    #
    cannibalismtext = "Cannibalism: In extreme situations, you may take serious decisions... If your food level is null, press X to " +\
                      "choose a crew member to eat."
    ecannibal = make_instruction(cannibalismtext)
    #
    #
    oasistext = "Oasis: During summer, you can find food and water in oasis'."
    ...
    #
    elements = [ecoord, efood, ewater, ehunt, eoasis, eforest, ejournal, evillage, ecamp, eflag, eseason, eweather, ecannibal, ewait,]
    thorpy.style.BOX_RADIUS += 10
